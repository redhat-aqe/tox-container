...
RUN /pyenv/bin/pyenv install 3.13.5
RUN echo 'command -v pyenv >/dev/null || export PATH="$PYENV_ROOT/bin:$PATH"' >> ~/.bashrc
RUN echo 'eval "$(/pyenv/bin/pyenv init -)"' >> ~/.bashrc && /pyenv/bin/pyenv global 3.13.5
RUN /pyenv/versions/3.13.5/bin/pip install awxkit tox
